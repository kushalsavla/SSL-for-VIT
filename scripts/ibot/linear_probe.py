#!/usr/bin/env python3
"""
Linear Probing for SSL-pretrained ViT-Small on CIFAR-10
Evaluates the quality of learned representations by training a linear classifier on frozen features.
"""
...
class CIFAR10Dataset(Dataset):
    """Simple CIFAR-10 dataset loader for linear probing."""
...
class LinearProbe(nn.Module):
    """Linear classifier on top of frozen SSL features."""
...
def load_pretrained_model(model_path, device):
    ...
    if os.path.exists(model_path):
        ...
        if 'student' in checkpoint:
            ...
            # Extract backbone weights (remove classifier weights)
            backbone_state = {}
...
def train_linear_probe(train_features, train_labels, val_features, val_labels, 
    ...
    # Create linear probe
    linear_probe = LinearProbe(feature_dim, num_classes).to(device)
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(linear_probe.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    ...
    for epoch in range(args.epochs):
        ...
        linear_probe.eval()
        with torch.no_grad():
            val_outputs = linear_probe(val_features)
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(linear_probe.state_dict(), args.output_dir + '/best_linear_probe.pth')
        
        if (epoch + 1) % 10 == 0:
            print(f"Epoch {epoch+1}/{args.epochs}: Loss={loss.item():.4f}, Val Acc={val_acc:.4f}")
    
    return linear_probe, best_acc, train_losses, val_accuracies

def evaluate_on_test(linear_probe, test_features, test_labels, device):
    """Evaluate the trained linear probe on test set."""
    ...
    with torch.no_grad():
        test_outputs = linear_probe(test_features)
...
def main():
    parser = argparse.ArgumentParser(description='Linear Probing for SSL-pretrained ViT')
    parser.add_argument('--data_path', type=str, default='../data/cifar10_splits/',
                       help='Path to CIFAR-10 splits')
    parser.add_argument('--model_path', type=str, default='./fine_tune_results/best_fine_tuned_model.pth',
    ...
    linear_probe, best_val_acc, train_losses, val_accuracies = train_linear_probe(
    ...
    test_acc, test_report = evaluate_on_test(linear_probe, test_features, test_labels, device)
