#!/usr/bin/env python3
"""
Non-linear Probing for Supervised ViT on CIFAR-10
Evaluates the expressiveness of learned features by training an MLP classifier on frozen backbone.
"""
...
class MLPProbe(nn.Module):
    """MLP classifier for non-linear probing."""
    def __init__(self, input_dim, hidden_dims=[512, 256], num_classes=10, dropout=0.2):
        super().__init__()
        layers = []
        prev_dim = input_dim
...
def train_nonlinear_probe(backbone, train_loader, val_loader, device, args):
    """Train an MLP classifier on frozen backbone features."""
    print("Training MLP classifier on frozen ViT features...")
    
    # Create MLP classifier
    classifier = MLPProbe(backbone.num_features, hidden_dims=args.hidden_dims, 
                         num_classes=10, dropout=args.dropout).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(classifier.state_dict(), os.path.join(args.output_dir, 'best_mlp_classifier.pth'))
            print(f"✅ New best model saved! (Val Acc: {val_acc:.2f}%)")
    
    return classifier, best_val_acc, train_losses, val_accuracies

def evaluate_nonlinear_probe(classifier, backbone, test_loader, device):
    """Evaluate the MLP classifier on test set."""
    print("Evaluating MLP classifier on test set...")
...
def main():
    parser = argparse.ArgumentParser(description='Non-linear probing for supervised ViT')
    ...
    classifier, best_val_acc, train_losses, val_accuracies = train_nonlinear_probe(
        backbone, train_loader, val_loader, device, args
    )
    training_time = time.time() - start_time
    
    # Evaluate on test set
    test_acc, test_report = evaluate_nonlinear_probe(classifier, backbone, test_loader, device)
    
    # Print summary
    print("\n" + "="*50)
    print("NON-LINEAR PROBING RESULTS (Supervised ViT)")
    print("="*50)
    ...
    with open(os.path.join(args.output_dir, 'nonlinear_probe_results.txt'), 'w') as f:
        f.write("NON-LINEAR PROBING RESULTS (Supervised ViT)\n")
        f.write("="*50 + "\n")
